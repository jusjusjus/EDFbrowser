...
int xml_attribute(const char *data, const char *item, char *result, int result_len)
{
	...
	if((data_len < 4) || (item_len >= (data_len - 4))) return -1;
...
char * xml_get_content_of_element(struct xml_handle *handle_p)
{
	...
	while(handle_p->child_handle_p != NULL) handle_p = handle_p->child_handle_p;
...
int xml_goto_next_element_with_same_name(struct xml_handle *handle_p)
{
	...
	while(1)
	{
		...
		if(handle_p->tag_search_result==NULL) return 1;
...
int xml_goto_next_element_at_same_level(struct xml_handle *handle_p)
{
	...
	while(handle_p->child_handle_p != NULL) handle_p = handle_p->child_handle_p;		// go to youngest child.
...
int xml_goto_nth_element_inside(struct xml_handle *handle_p, const char *name, int n)
{
	...
	while(handle_p->child_handle_p != NULL) handle_p = handle_p->child_handle_p;	// find the youngest child ???
...
void configpath(char* cfg_path, const char* filename)
{
#if defined(__linux__) || defined(__APPLE__)
	strcpy(cfg_path, getenv("HOME"));	// /home/jus
	strcat(cfg_path, "/.");			// /home/jus/.
	strcat(cfg_path, PROGRAM_NAME);		// /home/jus/.edfView
	strcat(cfg_path, "/");			// /home/jus/.edfView/
	mkdir(cfg_path,	S_IRWXU);
	strcat(cfg_path, filename);		// /home/jus/.edfView/filename
...
#endif
//	if(cfg_path[0] == 'a') std::cout << filename << std::endl;
}



FILE *open_configfile(const char* filename)
{
	char cfg_path[MAX_PATH_LENGTH];
	FILE *cfgfile=NULL;

	configpath(cfg_path, filename);
