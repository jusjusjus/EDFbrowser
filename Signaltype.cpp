...
Signaltype::Signaltype(QObject* theparent, QDomElement *signaltype) : name("Unspecified")
{
  ...
  if ( not signaltype->hasAttribute("Name") )
  {
    QMessageBox::information(parent->parent->mainwindow, tr("Signaltype"), tr("%1 has no Name attribute.").arg(signaltype->tagName()) );
    return;
  }
      ...
      child = child.nextSiblingElement("Event"))
  {
    ...
    {
      QMessageBox::information(parent->parent->mainwindow, tr("Signaltype"), tr("%1 has no Name attribute.").arg(child.tagName()) );
      continue;
    }
