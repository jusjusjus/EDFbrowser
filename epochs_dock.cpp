...
void UI_Epochswindow::load_epochinfo()
{
  ...
  if( child.isNull() or (not child.hasAttribute("Seconds")) )
  {
    QMessageBox::information(mainwindow, tr("Epochsconfig"), tr("Epoch is not properly defined in %1.").arg(cfg_path) );
    return;
  }
